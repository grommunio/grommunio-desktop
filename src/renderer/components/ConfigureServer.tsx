...
function ConfigureServer({
    ...
    const validateURL = async (url: string) => {
        ...
        if (validationResult?.status === URLValidationStatus.NotMattermost) {
            message = {
                type: STATUS.WARNING,
                value: formatMessage({id: 'renderer.components.configureServer.url.notMattermost', defaultMessage: 'The server URL provided does not appear to point to a valid grommunio server. Please verify the URL and check your connection.'}),
            };
        }
        ...
        if (validationResult?.status === URLValidationStatus.URLNotMatched) {
            message = {
                type: STATUS.WARNING,
                value: formatMessage({id: 'renderer.components.configureServer.url.urlNotMatched', defaultMessage: 'The server URL provided does not match the configured Site URL on your grommunio server. Server version: {serverVersion}'}, {serverVersion: validationResult.serverVersion}),
            };
        }
        ...
        if (validationResult?.status === URLValidationStatus.URLUpdated) {
            message = {
                type: STATUS.INFO,
                value: formatMessage({id: 'renderer.components.configureServer.url.urlUpdated', defaultMessage: 'The server URL provided has been updated to match the configured Site URL on your grommunio server. Server version: {serverVersion}'}, {serverVersion: validationResult.serverVersion}),
            };
        }
                                ...
                                <div className='ConfigureServer__card-form'>
                                    <Input
                                        ...
                                        customMessage={urlError ?? ({
                                            type: STATUS.INFO,
                                            value: formatMessage({id: 'renderer.components.configureServer.url.info', defaultMessage: 'The URL of your grommunio server'}),
                                        })}
