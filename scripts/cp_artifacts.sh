...
if [[ -f "${SRC}/grommunio-desktop-${VERSION}-win-arm64.zip" ]]; then
    echo -e "Copying Win64\n"
    cp "${SRC}/grommunio-desktop-${VERSION}-win-arm64.zip" "${DEST}/grommunio-desktop-${VERSION}-arm64.zip"
...
if [[ ${MM_WIN_INSTALLERS-0} -eq 1 && -f "${SRC}/grommunio-desktop-${VERSION}-x64.msi" ]]; then
    echo -e "Copying win-msi-x64\n"
    cp "${SRC}/grommunio-desktop-${VERSION}-x64.msi" "${DEST}/"
    SOMETHING_COPIED=$((SOMETHING_COPIED + 4))
fi
if [[ ${MM_WIN_INSTALLERS-0} -eq 1 && -f "${SRC}/grommunio-desktop-${VERSION}-x86.msi" ]]; then
    echo -e "Copying win-msi-x86\n"
    cp "${SRC}/grommunio-desktop-${VERSION}-x86.msi" "${DEST}/"
...
if [[ -f "${SRC}"/grommunio-desktop-${VERSION}-linux-x64.tar.gz ]]; then
    ...
    cp "${SRC}"/grommunio-desktop-*-linux-x64* "${DEST}/"
    cp "${SRC}"/grommunio-desktop-*-linux-x86_64* "${DEST}/"
    cp "${SRC}"/mattermost-desktop_"${VERSION}"-1_amd64*.deb "${DEST}/"
    SOMETHING_COPIED=$((SOMETHING_COPIED + 10))
fi

if [[ -f "${SRC}"/grommunio-desktop-${VERSION}-linux-arm64.tar.gz ]]; then
    echo -e "Copying linux\n"
    cp "${SRC}"/grommunio-desktop-*-linux-arm64* "${DEST}/"
    cp "${SRC}"/grommunio-desktop-*-linux-aarch64* "${DEST}/"
