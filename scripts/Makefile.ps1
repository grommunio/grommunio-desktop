...
function Run-BuildId {
    ...
    $ComponentDownload.InnerText = "https://releases.mattermost.com/desktop/$version/grommunio-desktop-$version-`$(var.Platform).msi"
    ...
    if (Test-Path 'env:PFX') {
        ...
        foreach ($archPath in "release\win-unpacked", "release\win-ia32-unpacked") {
            ...
            Get-ChildItem -Path $archPath -recurse "*.dll" | ForEach-Object {
                ...
                Start-Sleep -s 15
                signtool.exe sign /f "./grommunio-desktop-windows.pfx" /p "$env:PFX_KEY" /tr "http://timestamp.digicert.com" /fd sha1 /td sha1 "$($_.FullName)"
                Start-Sleep -s 15
            ...
            Start-Sleep -s 15
            signtool.exe sign /f "./grommunio-desktop-windows.pfx" /p "$env:PFX_KEY" /tr "http://timestamp.digicert.com" /fd sha1 /td sha1 "$archPath\grommunio.exe"
            Start-Sleep -s 15
            signtool.exe sign /f "./grommunio-desktop-windows.pfx" /p "$env:PFX_KEY" /tr "http://timestamp.digicert.com" /fd sha256 /td sha256 /as "$archPath\grommunio.exe"
        }
    } else {
...
function Run-BuildMsi {
    ...
    light.exe "scripts\msi_installer.wixobj" "scripts\msi_installer_files.wixobj" -loc "resources\windows\msi_i18n\en_US.wxl" -o "release\grommunio-desktop-$($env:COM_MATTERMOST_MAKEFILE_BUILD_ID)-x86.msi" -b "release\win-ia32-unpacked\"
    ...
    light.exe "scripts\msi_installer.wixobj" "scripts\msi_installer_files.wixobj" -loc "resources\windows\msi_i18n\en_US.wxl" -o "release\grommunio-desktop-$($env:COM_MATTERMOST_MAKEFILE_BUILD_ID)-x64.msi" -b "release\win-unpacked\"

    # Only sign the executable and .dll if this is a release and not a pull request
    # check.
    if (Test-Path 'env:PFX') {
        Print-Info "Signing grommunio-desktop-$($env:COM_MATTERMOST_MAKEFILE_BUILD_ID)-x86.msi (waiting for 15 seconds)..."
        Start-Sleep -s 15
        ...
        signtool.exe sign /f "./grommunio-desktop-windows.pfx" /p "$env:PFX_KEY" /tr "http://timestamp.digicert.com" /fd sha256 /td sha256 /d "release\grommunio-desktop-$($env:COM_MATTERMOST_MAKEFILE_BUILD_ID)-x86.msi" "release\grommunio-desktop-$($env:COM_MATTERMOST_MAKEFILE_BUILD_ID)-x86.msi"

        Print-Info "Signing grommunio-desktop-$($env:COM_MATTERMOST_MAKEFILE_BUILD_ID)-x64.msi (waiting for 15 seconds)..."
        Start-Sleep -s 15
        signtool.exe sign /f "./grommunio-desktop-windows.pfx" /p "$env:PFX_KEY" /tr "http://timestamp.digicert.com" /fd sha256 /td sha256 /d "release\grommunio-desktop-$($env:COM_MATTERMOST_MAKEFILE_BUILD_ID)-x64.msi" "release\grommunio-desktop-$($env:COM_MATTERMOST_MAKEFILE_BUILD_ID)-x64.msi"
    } else {
...
function Get-Cert {
    if (Test-Path 'env:PFX') {
        ...
        [IO.File]::WriteAllBytes("./grommunio-desktop-windows.pfx", [Convert]::FromBase64String($env:PFX))
        $password = "$env:PFX_KEY" | convertto-securestring -asplaintext -force
        Print-Info "Importing certificate into the machine"
        Import-PfxCertificate -filepath "./grommunio-desktop-windows.pfx" cert:\localMachine\my -password $password
    } else {
...
function Remove-Cert {
    if (Test-Path 'env:PFX') {
        ...
        Remove-Item -path "./grommunio-desktop-windows.pfx"
    }
}
